<template>
  <div class="containerGather">
    ...
    <div class="textwrap">
      ...
      <div class="ggog">
        ■ 풋살장 내 취사, 흡연 및 음주, 지나친 소음행위 금지 (적발 시 즉시 퇴장
        및 향후 이용불가)
      </div>
      <div class="ggog">
      ...
      <div class="ggog">
        ■ 위 내용이 지켜지지 않을 경우 환불 없이 퇴장조치 될 수 있으니 예약시 꼭
        참고
      </div>
    </div>
...
export default {
  ...
  created() {
    ...
    const API_URL = `${REST_API}/gather/gatherList/4`;
    axios({
      url: API_URL,
      method: "GET",
    })
  ...
  methods: {
    regist() {
      const REST_API = "http://localhost:9999";
      const API_URL = `${REST_API}/player/applyGame/4`;
